...
def open_booster(evt):
    ...
    yt = booster(value)
    C1 = deck.ImageTk.PhotoImage(deck.fr.card_model(deck.fr.nb_by_name(yt[0])))
    CL1.configure(image=C1)
    C2 = deck.ImageTk.PhotoImage(deck.fr.card_model(deck.fr.nb_by_name(yt[1])))
    CL2.configure(image=C2)
    C3 = deck.ImageTk.PhotoImage(deck.fr.card_model(deck.fr.nb_by_name(yt[2])))
